...
void userdgemm_nstn(const char *transa, const char *transb, const int *_m,
                    const int *_n, const int *_k, const double *_alpha,
                    const double *restrict A, const int *_lda,
                    const double *restrict B, const int *_ldb,
                    const double *_beta, double *restrict C, const int *_ldc);
...
void call_dgemm(CBLAS_LAYOUT layout, CBLAS_TRANSPOSE TransA,
    ...
    if (*_k == 40 && nota && !notb && *_alpha == -1.0 && *_beta == 1.0) {
        userdgemm_ksnt(transa, transb, _m, _n, _k, _alpha, A, _lda, B, _ldb, _beta, C, _ldc);
    }
    else if (*_n == 40 && !nota && notb && *_alpha == 1.0 && *_beta == 0.0) {
        userdgemm_nstn(transa, transb, _m, _n, _k, _alpha, A, _lda, B, _ldb, _beta, C, _ldc);
    }
    else {
        fprintf(stderr, "userdgemm_: not implemented error. userdgemm_(A:%s, B:%s, m:%d, n:%d, k:%d, alpha:%lf, lda:%d, ldb:%d, beta:%lf, ldc:%d)\n", transa, transb, *_m, *_n, *_k, *_alpha, *_lda, *_ldb, *_beta, *_ldc);
        exit(1);
    }
}
