...
static void micro_kernel0_ksnt(const double * restrict A, const double * restrict B,
...
static void micro_kernel1_ksnt(const double * restrict A, const double * restrict B,
                               double * restrict C) {
    register __m512d _A0, _A1;
    register __m512d _C0_0, _C1_0, _C2_0, _C3_0, _C4_0, _C5_0, _C6_0, _C7_0, _C8_0, _C9_0, _C10_0, _C11_0, _C12_0, _C13_0, _C14_0;
    register __m512d _C0_1, _C1_1, _C2_1, _C3_1, _C4_1, _C5_1, _C6_1, _C7_1, _C8_1, _C9_1, _C10_1, _C11_1, _C12_1, _C13_1, _C14_1;
...
static void inner_loop_ksnt(const uint64_t mm, const uint64_t nn, const double * restrict _A,
    ...
    for (uint64_t mmi = 0; mmi < mmc ; ++mmi) {
        ...
        for (uint64_t nni = 0; nni < nnc; ++nni) {
            ...
            if (mmm == MR && nnn == NR) {
                micro_kernel0_ksnt(_A + mmi * MR * KB, _B + nni * NR * KB, C + mmi * MR + nni * NR * ldc, ldc);
            } else {
                micro_kernel1_ksnt(_A + mmi * MR * KB, _B + nni * NR * KB, _C);
...
static void packacc_ksnt(uint64_t mm, const double * restrict A, uint64_t lda,
                        double * restrict _A) {
    uint64_t mmc = mm / MR;
    uint64_t mmr = mm % MR;
...
static void packbrr_ksnt(uint64_t nn, const double * restrict B, uint64_t ldb,
                         double * restrict _B) {
    uint64_t nnc = nn / NR;
    uint64_t nnr = nn % NR;
...
void userdgemm_ksnt(const char *transa, const char *transb, const int *_m,
    ...
    for(uint64_t mi = 0; mi < mc; ++mi) {
        ...
        packacc_ksnt(mm, A + mi * MB, lda, _A);
...
#pragma omp parallel
        {
            ...
            hbw_posix_memalign((void**)&_C, 64, sizeof(double) * MR * NR); 

#pragma omp for schedule(dynamic)
            for(uint64_t ni = 0; ni < nc; ++ni) {
                uint64_t nn = (ni + 1 < nc || nr == 0) ? NB : nr;

                packbrr_ksnt(nn, B + ni * NB, ldb, _B);
                inner_loop_ksnt(mm, nn, _A, _B, C + mi * MB + ni * NB * ldc, ldc, _C);
            }
