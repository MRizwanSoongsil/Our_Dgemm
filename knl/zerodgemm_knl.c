...
#define L1_DIST_B 60 //192 300



void micro_kernel0z(int k, const double * A, const double * B, double * C, int ncol)
{
  int i;
  ...
  register __m512d _C0_1, _C1_1, _C2_1, _C3_1, _C4_1, _C5_1, _C6_1, _C7_1, _C8_1, _C9_1, _C10_1, _C11_1, _C12_1, _C13_1, _C14_1;
    ...
    _C14_1 = _mm512_loadu_pd(&C[14*ncol+8]);
...
_mm512_storeu_pd(&C[14*ncol+0], _C14_0);
  _mm512_storeu_pd(&C[14*ncol+8], _C14_1);

}
void micro_kernel1z(int k, const double * A, const double * B, double * C, int ncol)
{
  int i;
  register __m512d _A0, _A1;
  register __m512d _C0_0, _C1_0, _C2_0, _C3_0, _C4_0, _C5_0, _C6_0, _C7_0, _C8_0, _C9_0, _C10_0, _C11_0, _C12_0, _C13_0, _C14_0;
		...
		double       *bk) // to *bk
{
	...
	int i, j;
	//omp_set_max_active_levels(2); // just in case
	// main part
	...
	//
	{
		...
		#pragma omp parallel num_threads(NT1) private(_B, nc, j)
		{
			...
			for(j = 0; j < nq; ++j)
			{
				...
				packbcrz(k,nc,&B[ki*KB+j*NB*lb],lb,_B);
				#pragma omp parallel num_threads(NT2) private(ir,mc,pq,p,pd,pc,_C) shared(_B, j,nc)
				{
					...
					for(ir = 0; ir <mq ; ++ir)
					{
						...
						for(p = 0; p < pq; ++p)
						{
							...
							if(pc == NR && mc == MR)
							{
								micro_kernel0z(k,&A[ir*MR*k],&_B[p*NR*k],&C[j*NB*lc+p*NR*lc+mi*MB+ir*MR],lc);
							}
							else
							{
								micro_kernel1z(k,&A[ir*MR*k],&_B[p*NR*k],_C,MR);
								micro_dxpyz(mc,pc,&C[j*NB*lc+p*NR*lc+mi*MB+ir*MR],_C,lc);
							}
						}
					}
				}
    ...
    //
    {
      // omp_set_max_active_levels(2);
        #pragma omp parallel num_threads(NT1) private(_B, nc, j)
        {
            ...
            {
                nc = (j != nq-1 || nd == 0) ? NB : nd;
                packbrrz(k,nc,&B[ki*KB*lb+j*NB],lb,_B);
                #pragma omp parallel num_threads(NT2) private(ir,mc,pq,p,pd,pc,_C) shared(_B, j,nc)
                {
                    #pragma omp for //schedule(dynamic)
                    // ir-loop
...
int kd = k % KB;
	...
	for(ii = 0; ii < mq; ++ii)
	{
		...
		for(l = 0; l < kq; ++l)
		{
			...
			packaccz(mc,kc,&A[ii*MB+l*KB*la],la,_A);
				jirloopTz(mc,n,ii,kc,l,_A,la,B,lb,C,lc);
		}
	}
}
